...
ML = 200000 #nombre d'echantillons aléatoire
t_init = 0
T  = 10
...
dWM=np.random.standard_normal((ML,5))
StNumML1=np.zeros((ML,6),'f')
StNumML1[:,:5]=dWM#First column corresponding to the wt  

StNumML1[:,5]=dW11#First column corresponding to the wt  
...
#Generator Network
NetworkG = Generator(input_neurons = 5, hidden_neurons = 200, output_neurons = 1)

#Discriminator Network
NetworkD = Discriminator(input_neurons = 6, hidden_neurons = 200, output_neurons = 1)

'''Optimizers'''

#Generator optimizer Network
optimizerG = torch.optim.Adam(NetworkG.parameters(), lr=0.00005, betas=(0.5, 0.999))
#Discriminator optimizer Network
optimizerD = torch.optim.Adam(NetworkD.parameters(), lr=0.00005, betas=(0.5, 0.999))
...
X_train_mini=torch.empty(batche_size, 5)
Y_train_mini=torch.empty(batche_size, 1)
Err_Proba=torch.zeros(0)
Err_Descri=torch.zeros(0)
Err_Genera=torch.zeros(0)
...
for epoch in range(30):
    ...
    for mini_batches in range(int(Ntrain/batche_size)):
       ...
       # Generator_loss = -loss_function(((Generator_Descriminator_out[:,0])), Y0)
        Generator_loss = torch.FloatTensor([0])
        
        Generator_loss[0] = -((Generator_Descriminator_out[:,0])).mean()
        #Generator_loss = ((Generator_Descriminator_out[:,0])).mean()
        ...
        #print('MiniBatxces=', mini_batches)
        ...
        for epoch2 in range(5):
            ...
            Descriminator_loss_1 = torch.FloatTensor([0])
            
            Descriminator_loss_1[0] = (Descriminator_real[:,0]).mean()
            #Descriminator_loss_1 = (Descriminator_real[:,0]).mean()
            
            #Descriminator_loss_1.backward(one)
            
            Generator_Descriminator_out = NetworkD(Generator_Descriminator_fake_in)
            
            Descriminator_loss_2 = torch.FloatTensor([0])

            Descriminator_loss_2[0] = (Generator_Descriminator_out[:,0]).mean()
            
            #Descriminator_loss_2 = (Generator_Descriminator_out[:,0]).mean()

            #Descriminator_loss_2.backward(mone)

            Descriminator_loss = Descriminator_loss_1 - Descriminator_loss_2
            
            Descriminator_loss.backward(mone)
            
            optimizerD.step()  
        
            #print('MiniBatxces2=', epoch2)
...
dWMT=np.random.standard_normal((ML,5))
StNumML1T=np.zeros((ML,6),'f')
StNumML1T[:,:5]=dWMT#First column corresponding to the wt  

StNumML1T[:,5]=dW1T#First column corresponding to the wt  
...
X_test_torch=torch.zeros(batchesize, 5)
Y_test_torch=torch.zeros(batchesize)
for i in range(30000):
  X_test_torch.copy_(torch.as_tensor(X_test[i*batchesize:(i+1)*batchesize,:5]))
  Y_test_torch.copy_(torch.as_tensor(Y_test[i*batchesize:(i+1)*batchesize]))
  Generator_test[i*batchesize:(i+1)*batchesize] = abs(NetworkG(X_test_torch))
